...
Int_t slcio2appTree(UInt_t nFirstJob, UInt_t nLastJob, const char * fn, const char * rfn)
{
	...
	TH1F histoCandidateM ("CandidateInvariantM", "CandidateInvariantM; M_{#gamma#gamma} (GeV)", 1000, 0, 1000);
	TH1F histoCandidatePt ("CandidatePt", "CandidatePt; Pt_{#gamma#gamma} (GeV)", 1500, 0, 1500);
	TH1F histoRemainingEnergy ("Remaining_Visible_Energy", "Remaining_Visible_Energy; E_{remaining visible} (GeV)", 1500, 0, 3000);
	TH1F histoCandidateEnergy ("CandidateE", "CandidateE; E_{#gamma#gamma} (GeV)",1500, 0, 1500);
	...
	TH1F histoconeEnergyFilter ("ConeEnergyFilter", "ConeEnergy; E_{cone} (GeV)", 100, 0, 10);
	...
	//TH1F thetaOfPhotons ("thetaOfPhotons", "thetaOfPhotons; #theta_{#gamma}", 90, 0, 180);
	TH1F PtofCandidatePhotons ("PtofCandidatePhotons", "PtofCandidatePhotons; Pt_{#gamma} (GeV)", 150, 0, 1500);
	TH1F energyofCandidatePhotons ("energyofCandidatePhotons", "energyofCandidatePhotons; E_{#gamma} (GeV)", 150, 0, 1500);
	...
	TH1F histoXPhotonsPerEvent ("x_photons_per_event", "three_photons_per_event; 2#theta_{#gamma}", 90, 0, 180);
	TH1F histoSmallestAngleofXPhotons ("smallest_angle_of_x_photons", "smallest_angle_of_2_photons; 2#theta_{#gamma}", 90, 0, 90);
	TH1F histoEnergyXPhotons ("energy_of_x_photons", "energy; E_{#gamma}", 1500, 0, 1500);
	TH1F histoConeParticlesPDG ("ParticleConePdg", "ParticleConePdg", 1000, 0, 1000);
	TH1F histoThetaCandidatesw3Photons("ThetaCandidatesw3Photons","ThetaCandidatesw3Photons; #theta_{#gamma#gamma}", 90,0,180);
	...
	Int_t counterconeEnergy01=0;
	Int_t counterconeEnergy05=0;
	Int_t counterconeEnergy1=0;
	Int_t counterconeEnergy2=0;
	Int_t counterconeEnergy4=0;
	Int_t counterconeEnergy6=0;
	Int_t counterconeEnergy8=0;
	Int_t counterconeEnergy10=0;
	...
	histo2ndHighestPhotonPtZoomed.Fill(PtPhotons[1]);
					...
					//	Double_t coneEnergy = 0; //Energija konusa oko fotona
						...
						{
							IMPL::ReconstructedParticleImpl* recoParticle = (IMPL::ReconstructedParticleImpl*) recParticles->getElementAt(k);
							const double *impuls = recoParticle->getMomentum();
							double energija = recoParticle->getEnergy();
							int PDG = recoParticle->getType();
							...
							if (particleAngle <=coneAngle)
							{
								...
								histoConeParticlesPDG.Fill(PDG);
							}
			...
			vector <Double_t> AngleOfXPhotonsPerEvent;
			vector <Double_t> vectorEnergyOfXPhotons;

			if (photons.size() == 3)
			{


				for (int k = 0; k < (int)photons.size() - 1; ++k)
				{
				histoXPhotonsPerEvent.Fill(photons[k].Theta()*180/M_PI);
				Double_t theta2Photons = photons[k].Theta();
				Double_t energyOfXPhotons = photons[k].E();
				vectorEnergyOfXPhotons.push_back(energyOfXPhotons);
				if ( theta2Photons <= M_PI/2)
				{
					theta2Photons= photons[k].Theta()*180/M_PI;
				} else
				{
					theta2Photons= (M_PI-photons[k].Theta())*180/M_PI;
				}
				AngleOfXPhotonsPerEvent.push_back(theta2Photons);


				}

				sort(AngleOfXPhotonsPerEvent.begin(), AngleOfXPhotonsPerEvent.end(), less<int>());
				histoSmallestAngleofXPhotons.Fill(AngleOfXPhotonsPerEvent[0]);

				sort(vectorEnergyOfXPhotons.begin(),vectorEnergyOfXPhotons.end(), greater<int>());
				histoEnergyXPhotons.Fill(vectorEnergyOfXPhotons[0]);
		...
		//	cout << "photon size is: " << photons.size()<<endl;
				...
				for (int i = 0; i < (int)photons.size() - 1; ++i)
				{
					for (int j = i + 1; j < photons.size(); ++j)
					{
						...
						if (minInvMass < pair.M() && pair.M() < maxInvMass)//uslov da bi par bio higs kandidat
						{
							...
							histoThetaCandidatesw3Photons.Fill(pair.Theta());
			...
			//		cout << "E remaining:"<< Eremaining<<endl;
				...
				//	Double_t angleBetweenPotons = candidate->Photon1.Angle(candidate->Photon2.Vect());
					...
					if (cosinusHelicity < cutCosinusHelicity)
					{
						counterCosinusHelicity++;
					}
	...
	Double_t percentageEremaining = (Double_t) counterEremaining/totalCutEvents * 100;
	Double_t percentageCosinusHelicity = (Double_t) counterCosinusHelicity/totalCutEvents * 100;
	Double_t percentageEhiggs = (Double_t) counterEhiggs/totalCutEvents * 100;
	Double_t percentageHiggsPt = (Double_t) counterHiggsPt/totalCutEvents * 100;
	Double_t percentagePreselectionCuts = (Double_t) counterPreselectionCuts/totalCutEvents * 100;
	Double_t percentageHiggsInvMass = (Double_t) counterHiggsInvMass/totalCutEvents * 100;


	Double_t percentageConeEnergy01= (Double_t) counterconeEnergy01/totalEvents * 100;
	Double_t percentageConeEnergy05= (Double_t) counterconeEnergy05/totalEvents * 100;
	Double_t percentageConeEnergy1= (Double_t) counterconeEnergy1/totalEvents * 100;
	Double_t percentageConeEnergy2= (Double_t) counterconeEnergy2/totalEvents * 100;
	Double_t percentageConeEnergy4= (Double_t) counterconeEnergy4/totalEvents * 100;
	Double_t percentageConeEnergy6= (Double_t) counterconeEnergy6/totalEvents * 100;
	Double_t percentageConeEnergy8= (Double_t) counterconeEnergy8/totalEvents * 100;
	Double_t percentageConeEnergy10= (Double_t) counterconeEnergy10/totalEvents * 100;
	...
	coneEnergyEff.open("InvMconeEnergyEff.txt",ios_base::out | ios_base::app);
	coneEnergyEff<< "0.1 cone Energy:"<<percentageConeEnergy01<<endl;
	coneEnergyEff<< "0.5 cone Energy:"<<percentageConeEnergy05<<endl;
	coneEnergyEff<< "1 cone Energy:"<<percentageConeEnergy1<<endl;
	coneEnergyEff<< "2 cone Energy:"<<percentageConeEnergy2<<endl;
	coneEnergyEff<< "4 cone Energy:"<<percentageConeEnergy4<<endl;
	coneEnergyEff<< "6 cone Energy:"<<percentageConeEnergy6<<endl;
	coneEnergyEff<< "8 cone Energy:"<<percentageConeEnergy8<<endl;
	coneEnergyEff<< "10 cone Energy:"<<percentageConeEnergy10<<endl;
...
/*	histoCandidatePhi.Write();
	...
	histoXPhotonsPerEvent.Write();
	histoSmallestAngleofXPhotons.Write();
	histoEnergyXPhotons.Write();
	histoThetaCandidatesw3Photons.Write();
...
/*	TCanvas c1;
	...
	histoXPhotonsPerEvent.Draw();
	c45.Print("histoXPhotonsPerEvent.pdf");
	...
	histoSmallestAngleofXPhotons.Draw();
	c46.Print("histoSmallestAngleofXPhotons.pdf");
	...
	//c33.SetTitle("Signal");
	//c46.GetPad(1)->SetLogy();
	histoEnergyXPhotons.Draw();
	c47.Print("histoEnergyXPhotons.pdf");
	...
	//c33.SetTitle("Signal");
	//c46.GetPad(1)->SetLogy();
	histoConeParticlesPDG.Draw();
	c48.Print("ConeParticlePDG.pdf");

	TCanvas c49;
	gStyle->SetPalette( 1 );
	gStyle->SetOptStat(111111);
	c49.SetCanvasSize(1000,650);
	c49.Divide(1,1,0.01,0.01);
	c49.cd(1);
	//c33.SetTitle("Signal");
	//c46.GetPad(1)->SetLogy();
	histoThetaCandidatesw3Photons.Draw();
	c49.Print("ThetaCandidatesw3Photons.pdf");*/
...
Int_t main(int argc, char* argv[])
{
	...
	TString rfName = "coneisolation_with_isolation_test.root";
