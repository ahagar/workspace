...
Int_t slcio2appTree(UInt_t nFirstJob, UInt_t nLastJob, const char * fn, const char * rfn)
{
	...
	TH1F histoRemainingEnergy ("Remaining_Visible_Energy", "Remaining_Visible_Energy; E(GeV)", 150, 0, 3000);
	...
	TH1F histoHigherEnergyPhoton ("HigherEnergyPhoton", "HigherEnergyPhoton; E_{#gamma1}", 800, 0, 800);
	TH1F histoLowerEnergyPhoton ("LowerEnergyPhoton", "LowerEnergyPhoton; E_{#gamma2}", 80, 0, 800);
	...
	TH1F histoconeEnergy ("Cone Energy", "Cone Energy; E (GeV)", 100, 0, 100);
	...
	TH1F histoTheta1Photon ("histoTheta", "Theta of candidate Photon; #theta_{#gamma}", 50, 0, 3.14);
	...
	TH1F histoPtof1stPhotonofCandidate ("Ptof1stPhotonofCandidate", "Pt of 1st Photon of Candidate; Pt (GeV)", 250, 0, 500);
	...
	TH1F CandidateEnergyZoom ("CandidateEnergyZoom", "CandidateEnergyZoom; E_{H} (GeV)",200, 600, 800);
	...
	TH1F proveraPhoton1Energy ("EnergyPhoton1", "proveraPhoton1Energy; E_{#gamma1}", 300, 0, 600);
	TH1F proveraPhoton2Energy ("EnergyPhoton2", "proveraPhoton2Energy; E_{#gamma2}", 300, 0, 600);
	TH1F proveraCandidateEnergy ("proveraCandidateEnergy", "proveraCandidateEnergy; E_{#gamma#gamma}", 150, 600, 750);
	...
	for(UInt_t iJob=nFirstJob; iJob<=nLastJob; iJob++)
	{
		...
		while( (evt = lcReader->readNextEvent()) != 0 /*&& brojac < 1000*/)
		{
			...
			IMPL::LCCollectionVec* recParticles = (IMPL::LCCollectionVec*)evt->getCollection("PandoraPFOCollection");
			...
			{
				IMPL::ReconstructedParticleImpl* mcParticle = (IMPL::ReconstructedParticleImpl*) recParticles->getElementAt(i);
				...
				Int_t particlePDG = fabs(mcParticle->getType());
		...
		//	cout << "photon size is: " << photons.size()<<endl;
			...
			//	histoNumberPhotonsbyEvent.Fill(photons.size());//histogram koji nam pokazuje broj fotona po događaju
...

				{
					...
					if (650 < higgs.E() && higgs.E() < 710 )
					{
						Double_t vecaEnergija = TMath::Max(photon.E(), photon2.E());
						Double_t nizaEnergija = TMath::Min(photon.E(), photon2.E());

						proveraPhoton1Energy.Fill(vecaEnergija);
						proveraPhoton2Energy.Fill(nizaEnergija);
						proveraCandidateEnergy.Fill(higgs.Energy());
					}
				...
				//	Double_t angleBetweenPotons = candidate->Photon1.Angle(candidate->Photon2.Vect());
					...
					CandidateEnergyZoom.Fill(higgs.Energy());
	...
	CandidatePhotonEnergy1.Write();
	CandidatePhotonEnergy2.Write();
	proveraPhoton1Energy.Write();
	proveraPhoton2Energy.Write();
	...
	c4.Print("histoRemainingEnergy.eps");
	...
	c5.Print("histoCandidateEnergy.eps");
	...
	c6.Print("histoCandidateTheta.eps");
	...
	c7.Print("histoCandidatePhi.eps");
	...
	c8.GetPad(1)->SetLogy();
	histoBoost.Draw();
	c8.Print("histoCandidateBoost.eps");
	...
	c9.Print("histoHigherEnergyPhoton.eps");
	...
	c10.Print("histoLowerEnergyPhoton.eps");
	...
	c12.Print("histoHelicityAngle.eps");
	...
	c17.Print("angleBetweenPhotons.eps");
	...
	c18.Print("NumberPhotonsbyEvnt.eps");
	...
	c19.Print("PhotonParticleAngle.eps");
	...
	c20.Print("PhotonParticleAngleWE.eps");
	...
	c21.Print("Cone_Energy_Filter.eps");
	...
	c22.Print("HardPhotonsByEvent.eps");
	...
	c23.Print("histoPtOtherParticles.eps");
	...
	c24.Print("histoMissingEnergy.eps");
	...
	c26.Print("efikasnostPhotonPT.eps");
	...
	c27.Print("Thetaof1stPhotonofCandidate.eps");
	...
	c28.Print("Thetaof2ndPhotonofCandidate.eps");
	...
	c29.Print("Ptof1stPhotonofCandidate.eps");
	...
	c30.Print("Ptof2ndPhotonofCandidate.eps");
	...
	//c28.SetTitle("Signal");
		//c24.GetPad(1)->SetLogy();
	histoPtof2ndPhotonofCandidate.Draw();
	c31.Print("histoHighestPhotonPt.eps");
	...
	c32.Print("histo2ndHighestPhotonPt.eps");
	...
	c33.Print("histo2ndHighestPhotonPtZoomed.eps");
	...
	c34.Print("histoVisibleEnergy.eps");
	...
	c35.Print("histoZbirImpulsa.eps");
	...
	c36.Print("cosinusHelicityAngle.eps");
	...
	//c33.SetTitle("Signal");
		//c33.GetPad(1)->SetLogy();
	CandidateEnergyZoom.Draw();
	c37.Print("CandidateEnergyZoom.eps");
	...
	c38.Print("CandidatePhotonEnergy1.eps");
	...
	c39.Print("CandidatePhotonEnergy2.eps");
	...
	//c33.SetTitle("Signal");
		//c33.GetPad(1)->SetLogy();
	proveraPhoton1Energy.Draw();
	c40.Print("proveraPhoton1Energy.eps");
	...
	//c33.SetTitle("Signal");
		//c33.GetPad(1)->SetLogy();
	proveraPhoton2Energy.Draw();
	c41.Print("proveraPhoton2Energy.eps");
	...
	proveraCandidateEnergy.Draw();
	c42.Print("proveraCandidateEnergy.eps");
...
Int_t main(int argc, char* argv[])
{
	...
	TString rfName = "pfoCollection.root";
