...
Double_t coneAngle = 2.5;		   //ugao konusa
Double_t coneAngle5 = 2.0;		   //ugao konusa
Double_t coneAngle20 = 3.0;		   //ugao konusa
...
Int_t slcio2appTree(UInt_t nFirstJob, UInt_t nLastJob, const char * fn, const char * rfn)
{
	...
	TH1F histoNumberPhotonsbyEvent ("NumberofPhotonsbyEvent", "NumberofPhotonsbyEvent", 10, 0, 10);
	TH1F histoNumberPhotonsbyEvent5 ("NumberofPhotonsbyEvent5", "NumberofPhotonsbyEvent5", 10, 0, 10);
	TH1F histoNumberPhotonsbyEvent20 ("NumberofPhotonsbyEvent20", "NumberofPhotonsbyEvent20", 10, 0, 10);
	...
	TH1F histoPtOtherParticles ("PtOtherParticles", "PtOtherParticles; Pt (GeV)", 1500, 0, 1500);
	...
	varListGoran vl;
	eventList.Branch("CandidateM", &(vl.CandidateM), "CandidateM/F" );
	eventList.Branch("CandidateE", &(vl.CandidateEnergy), "CandidateE/F" );
	eventList.Branch("CandidatePt", &(vl.CandidatePt), "CandidateE/F" );
	eventList.Branch("CandidateEnergy", &(vl.CandidateEnergy), "CandidateEnergy/F" );
	eventList.Branch("CandidateTHeta", &(vl.CandidateTHeta), "CandidateTHeta/F" );
	eventList.Branch("secondHighestPt", &(vl.secondHighestPt), "secondHighestPt/F" );
	eventList.Branch("pTHiggsPhoton1", &(vl.pTHiggsPhoton1), "pTHiggsPhoton1/F" );
	eventList.Branch("pTHiggsPhoton2", &(vl.pTHiggsPhoton2), "pTHiggsPhoton2/F" );

	Float_t cone14, cone20,cone5;

	TTree coneEnergyList("cone", "ILD event list");
	coneEnergyList.Branch("cone14", &cone14, "cone14/F");
	coneEnergyList.Branch("cone20", &cone20, "cone20/F");
	coneEnergyList.Branch("cone5", &cone5, "cone5/F");
	...
	for(UInt_t iJob=nFirstJob; iJob<=nLastJob; iJob++)
	{
		...
		while( (evt = lcReader->readNextEvent()) != 0 /*&& brojac < 1000*/)
		{
			...
			vector<TLorentzVector> photons5;
			vector<TLorentzVector> photons20;
			...
			//	cout<<"particle PDG is: "<<particlePDG<<endl;
				...
				//	Double_t Pt = temp.Pt();		//promenljiva koja nam daje Pt čestice
					...
					if(temp.Pt() >minPt1 /*minPt1*/)
					{
						...
						Double_t coneEnergy20 = 0; //Energija konusa oko fotona
						...
						if(coneEnergy5 <= maxConeEnergy){
							photons5.push_back(temp);
						}
						if(coneEnergy20 <= maxConeEnergy){
							photons20.push_back(temp);
						}
				...
				//dogadjaj ne prolazi ako se u dogadjaju detektuje lepton
					...
					if(temp.Pt() > 15 && fabs(particlePDG) == 11)
						leptonFound = true;
			...
			histoNumberPhotonsbyEvent5.Fill(photons5.size());
			histoNumberPhotonsbyEvent20.Fill(photons20.size());
		...
		//	cout << "photon size is: " << photons.size()<<endl;
			...
			{
				if(leptonFound) brojacLeptonFound++;
				...
				//	if (PtPhotons[1] > 15) { }
						...
						vl.secondHighestPt = PtPhotons[1];
						vl.pTHiggsPhoton1 = photon.Pt();
	...
	cout << "brojacLeptonFound je: " << brojacLeptonFound << endl;
...
//	cout << "Efikasnost je: " << percentagePt4 << " %"<< endl;
    ...
    histoPtOtherParticles.Write();
	anglePhotonParticlewE.Write();
	anglePhotonParticle.Write();
	histoNumberPhotonsbyEvent5.Write();
	histoNumberPhotonsbyEvent.Write();
	histoNumberPhotonsbyEvent20.Write();
    eventList.Write();
    coneEnergyList.Write();
...
/*	TCanvas c18;
	...
	anglePhotonParticlewE.Draw();	anglePhotonParticle.Draw();
