...
//Two photons + invM
	...
	/*		IMPL::LCCollectionVec* eCalBarrelHits = (IMPL::LCCollectionVec*)evt->getCollection("ECALBarrel_Reprocess");
			...
			for (Int_t i = 0; i < recParticles->getNumberOfElements() ; i++)
			{
				...
				TLorentzVector modifiedVec; //četvorovektor u koji sakupljamo informacije o svakoj čestici
			...
			//	correction = (particlePDG == 22) ?  correction : 0;
				...
				float corr_px = temp.Px()+ 0.1104 - 2*rnd_px.Integer(2)*0.1104  ;
				float corr_py = temp.Py()+ 0.004 - 2*rnd_py.Integer(2)*0.004 ;
				float corr_pz = temp.Pz()+ 0.292 - 2*rnd_pz.Integer(2)*0.292 ;

				double corr_e =sqrt(corr_px * corr_px + corr_py* corr_py + corr_pz * corr_pz);

				double s_f = corr_e/e;//faktor skaliranja da vektori budu isti

				temp.SetPxPyPzE(s_f*p[0], s_f*p[1], s_f*p[2], s_f*e); //novi 4Vektor kom su promenjene vrednosti za +/-
...
//					cout<<"theta je: "<< theta<<endl;
				...
				//	Double_t Pt = temp.Pt();		//promenljiva koja nam daje Pt čestice
					...
					//	cout <<"vrednosti novog 4 Vektora su px: "<<modifiedVec.Px()<<", py: "<< modifiedVec.Py()<<" ,pz: "<<modifiedVec.Pz()<<" ,E: "<<modifiedVec.E()<<endl;
					//	cout <<"vrednosti temp 4 Vektora su px: "<<temp.Px()<<", py: "<< temp.Py()<<" ,pz: "<<temp.Pz()<<" ,E: "<<temp.E()<<endl;
						...
						for (Int_t k = 0; k < recParticles->getNumberOfElements() ; k++) /*&& coneEnergy <= maxConeEnergy -zaustavlja fot loop kada predje maxE */
						{
							...
							if (fabs(recParticle->getType())==22)
							{
								otherParticle.SetPxPyPzE(s_f*impuls[0], s_f*impuls[1], s_f*impuls[2], s_f*energija);
							}
							...
							float corr_pxo = otherParticle.Px()+ 0.1104 - 2*rnd_pxo.Integer(2)*0.1104  ;
							float corr_pyo = otherParticle.Py()+ 0.004 - 2*rnd_pyo.Integer(2)*0.004 ;
							float corr_pzo = otherParticle.Pz()+ 0.292 - 2*rnd_pzo.Integer(2)*0.292 ;

							float corr_eo = sqrt(corr_pxo * corr_pxo + corr_pyo*corr_pyo + corr_pzo*corr_pzo);

							if (fabs(recParticle->getType())==22)
							{
							otherParticle.SetPxPyPzE(corr_pxo*impuls[0], corr_pyo*impuls[1], corr_pzo*impuls[2], corr_eo);
							}	else {otherParticle.SetPxPyPzE(impuls[0], impuls[1], impuls[2], energija);
}
						...
						//	cout<<"ugao izmedju čestica: "<<particleAngle<<endl;
...
Int_t main(int argc, char* argv[])
{
	...
	TString rfName = "modifiedVector_app.root";
