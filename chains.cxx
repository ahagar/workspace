...
//--------------------------------------------------------------------------------
EVENT::MCParticle*  findParticleInChain (const EVENT::MCParticle*  mcParticle, int pdg);

EVENT::MCParticle*  findElectronInChain (const EVENT::MCParticle*  mcParticle)
{
	findParticleInChain(mcParticle, 11);
}

EVENT::MCParticle*  findPositronInChain (const EVENT::MCParticle*  mcParticle)
{
	findParticleInChain(mcParticle, -11);
}
...
EVENT::MCParticle*  findParticleInChain (const EVENT::MCParticle*  mcParticle, int pdg)
{
	cout << "findInChain: pdg " << mcParticle->getPDG() << ", " << mcParticle->getEnergy() << endl;
	...
	// check if final
	if (cerke.size() == 0) {
		if (mcParticle->getPDG() == pdg && mcParticle->getGeneratorStatus() == 1) {
			return const_cast<EVENT::MCParticle*>(mcParticle);
		}
	}

	for (int l = 0; l < (int) cerke.size(); l++)
	{
		const EVENT::MCParticle* cerka = cerke[l];

		if (abs(cerka->getPDG()) == 11 || cerka->getPDG() == 94)
		{
			EVENT::MCParticle* found = findParticleInChain (cerka, pdg);
			if (found) {
				return found;
			}
		}
...
Int_t slcio2appTree(UInt_t nFirstJob, UInt_t nLastJob, const char * fn, const char * rfn)
{
	...
	for(UInt_t iJob=nFirstJob; iJob<=nLastJob; iJob++)
	{
		...
		EVENT::LCEvent* evt = 0;
		while( (evt = lcReader->readNextEvent()) != 0 /*&& brojac < 3*/)
		{
		...
		//	cout << "usao u while " <<endl;
			...
			//		cout << "usao u if " <<endl;
					...
					EVENT::MCParticle* firstParent = mcParticle -> getParents()[0];
					EVENT::MCParticle* foundElectron = findElectronInChain(firstParent);
					EVENT::MCParticle* foundPositron = findPositronInChain(firstParent);

					if (foundElectron){
						cout << "nasao sam finalni elektron sa pdg i gen statusom: "<<
								foundElectron->getPDG() <<", "<< foundElectron->getEnergy() <<endl;
					}

					if (foundPositron){
				//		cout << "nasao sam finalni pozitron sa pdg i gen statusom: "<<
							//	foundPositron->getPDG() <<", "<< foundPositron->getEnergy() <<endl;
					}
