...
Int_t slcio2appTree(UInt_t nFirstJob, UInt_t nLastJob, const char * fn, const char * rfn)
{
	...
	TH1F histoPtof1stPhotonofCandidate ("Ptof1stPhotonofCandidate", "Ptof1stPhotonofCandidate; Pt (GeV)", 500, 0, 1000);
	TH1F histoPtof2ndPhotonofCandidate ("Ptof2ndPhotonofCandidate", "Pt of 2nd Photon of Candidate; Pt (GeV)", 500, 0, 1000);
...
//Two photons + invM
	...
	//two  Photons with pt>10, upisuje se za sve evente
		...
		TH1F histoECall_def_preselection ("ECall_def_preselection", "ECall; E_{ecal} (GeV)", 3000, 0, 3000);
		TH1F histoHCall_def_preselection ("HCall_def_preselection", "HCall; E_{hcal} (GeV)", 3000, 0, 3000);
		TH1F histoTotalCall_def_preselection  ("EnergyCalorimeter_def_preselection ", "EnergyCalorimeter; E_{calorimeter} (GeV)", 300, 0, 3000);
		TH1F histoCallRatio_def_preselection  ("calRatio_def_preselection ", "calRatio ", 100, 0, 1);
		TH1F histoECall_def_candidate  ("ECall_def_candidate ", "ECallEvt; E_{ecal} (GeV)", 3000, 0, 3000);
		TH1F histoHCall_def_candidate  ("HCall_def_candidate ", "HCallEvt; E_{hcal} (GeV)", 3000, 0, 3000);
		TH1F histoECallEvt_def_candidate  ("ECallEvt_def_candidate ", "ECallEvt; E_{ecal} (GeV)", 3000, 0, 3000);
		TH1F histoHCallEvt_def_candidate  ("HCallEvt_def_candidate ", "HCallEvt; E_{hcal} (GeV)", 3000, 0, 3000);
		TH1F histoTotalCallEvt_def_preselection  ("EnergyCalorimeterEvt_def_preselection ", "EnergyCalorimeterEvt; E_{calorimeter} (GeV)", 300, 0, 3000);
		...
		TH1F histopLPhotonsCandidate ("longitudinal_momentum_of_candidate_photons", "pL; p_{L} (GeV)", 3000, 0, 3000);
	...
	for(UInt_t iJob=nFirstJob; iJob<=nLastJob; iJob++)
	{
		...
		EVENT::LCEvent* evt = 0;
		while( (evt = lcReader->readNextEvent()) != 0 /*&& brojac < 10*/)
		{
			...
			vector<TLorentzVector> candidatePhotons;

			vector<Double_t> photonsECal;
			vector<Double_t> photonsHCal;
			vector<Double_t> photonsECalCandidate;
	...
	histo2ndHighestPhotonPtZoomed.Fill(PtPhotons[1]);
						...
						{
							candidatePhotons.push_back(temp);//cuvamo fotone u niz
							histoconeEnergyFilter.Fill(coneEnergy);//histogram koji iscrtava energiju konusa

							//cout<< "candidate photons size je :  "<< candidatePhotons.size()<<endl;
							...
							photonsECalCandidate.push_back(ecal);
				...
				/*if (photons.size()==2 && recParticle->getType()==22)
					{
						//cout<<"redni broj događaja:"<< brojDogadjaja<<", čestica upisana"<< recParticle->getType()<<endl;
						std::vector<EVENT::Cluster*> clusters = (std::vector<EVENT::Cluster*>) recParticle->getClusters();
						 for ( std::vector<EVENT::Cluster*>::const_iterator iCluster=clusters.begin(); iCluster!=clusters.end(); ++iCluster)
						{
...
//			histoECallEvt_pt_preselection.Fill(ecalEvt);
			...
			if (candidatePhotons.size() == 2)
				{
				if(photonsECalCandidate[0]<2900 && photonsECalCandidate[1]<2900)
				{


					photonsECal.push_back(ecal);//energija pojedinacnih fotona iz ecala
					photonsHCal.push_back(hcal);
				photons = candidatePhotons;

				}
				//cout<< "photons size je :  " << photons.size()<<endl;
			//	cout<<"ecal 1 :"<<photonsECalCandidate[0]<<endl;
				//cout<<"energija  jke :"<<candidatePhotons[0].E()<<endl;
	...
	//		histoMissingEnergy.Fill(Emiss);
		...
		//	cout << "photon size is: " << photons.size()<<endl;
			...
			if (photons.size() == photonSize )
			{
				...
				double totalCal2Photons = 0;
				double ecal2Photons = 0;
				for(int i = 0; i < photons.size(); ++i)
				{
					double ecal = photonsECal[i];
					double hcal = photonsHCal[i];
					ecal2Photons +=ecal;
					histoECall_def_preselection.Fill(ecal);
					histoHCall_def_preselection.Fill(hcal);
				}

				totalCal2Photons+=ecal + hcal;

				vl.EcallEnergy = ecal;
				vl.HcallEnergy = hcal;
				vl.TotalcallEnergy = totalCal2Photons;
				 calRatio = ecal/totalCal;
				 histoCallRatio.Fill(calRatio);

				histoECall_def_candidate.Fill(ecal2Photons);
				histoHCall_def_candidate.Fill(hcalEvt);
				histoTotalCallEvt_def_preselection.Fill(totalCalEvt);
				histoTotalCall_def_preselection.Fill(totalCal2Photons);
				histoTotalCallEvt_def_2photons.Fill(totalCal2Photons);
				...
				//	numCutEvents++;//brojimo dogadjaje koji prodju uslove za energiju, invarijantnu masu, uglove,Pt
				    ...
				    histopLPhotonsCandidate.Fill(abs(photon.Z()));
				    histopLPhotonsCandidate.Fill(abs(photon2.Z()));
	...
	cout << "Broj događaja posle ecal < 600: " << CounterStrike << endl;
	...
	ofstream file ;
	file.open("probaEcal.txt", ios_base::out | ios_base::app);
	...
	preselectionEfficiency.open( "preselectionEcal.txt " ,ios_base::out | ios_base::app);
...
/*	histoCallRatio_ecal_preselection.Write();
	...
	//after definition
	histoECall_def_preselection.Write();
	histoHCall_def_preselection.Write();
	histoECall_def_candidate.Write();
	histoHCall_def_candidate.Write();
	histoTotalCallEvt_def_preselection.Write();
	histoTotalCall_def_preselection.Write();
	histoTotalCallEvt_def_2photons.Write();
	histoECallEvt_photons_preselection.Write();

	histopL.Write();
	histoCandidatePtRange.Write();
	histopLRange.Write();
	histopLPhotonsCandidate.Write();
...
Int_t main(int argc, char* argv[])
{
	...
	TString rfName = "ecalcandidateVarTestEcal3000.root";
