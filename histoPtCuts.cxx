...
Int_t slcio2appTree(UInt_t nFirstJob, UInt_t nLastJob, const char * fn, const char * rfn)
{
	...
	TH1F NumberOfPhotonByEvent ("Photon No by event", "Photon No by event;", 10, 0, 10);
    ...
    //       bool leptonFound = false;
		...
		//	sort(PtPhotons.begin(), PtPhotons.end(), greater<int>());
            ...
            if (photons.size() > 1)
				...
				NumberOfPhotonByEvent.Fill(photons.size());
	...
	ofstream results ;
	results.open("pTCuts.txt", ios_base::out | ios_base::app);
...
//	c1.GetPad(1)->SetLogy();
	...
	//c2.GetPad(1)->SetLogy();
	histoPt2Max.Draw();
	c2.Print("pTCutshisto2ndMax.eps");
	...
	c3.GetPad(1)->SetLogy();
	histoPtMaxZoom.Draw();
	c3.Print("pTCutshistoMaxPtZoom.eps");
	...
	c4.Print("pTCutshisto2ndMaxZoom.eps");
	...
	c6.Print("pTCutsPhotonTheta.eps");
...
Int_t main(int argc, char* argv[])
{
	...
	TString rfName = "histoPtCuts.root";
