...
/*	leptonTree.Branch("Energija_konusa", &Energija_konusa, "Energija_konusa");
	...
	Float_t  thetaL;
		Float_t pid_leptona;
		Float_t el_ph_angle;
		Float_t ratio_en_emu;
			//Float_t d0_recparticle, z0_recparticle, r03d_recparticle;
		Float_t n_ratio;
		//	Float_t n_svipfo;
			Float_t ratio_en_pfo;
...
//			Float_t Brojac_za_dvicu = 0;
			...
			//	if (mcParticle -> getGeneratorStatus() != 1) continue;
				...
				if (fabs(PDG) == 11 && temp.E() > 60 && mcParticle -> getGeneratorStatus() == 1)
				{
					...
					Theta_KG_elektrona = temp.Theta() * 180/M_PI;
 					...
 					for (int j = 0; j < links -> getNumberOfElements(); j++)
 					{
						...
						if (link_to_rec_lepton1)
							...
							if (rpi1 -> getType() == 11 && rpi1 -> getEnergy() > 60)
							{
								elektronac = true;
							...
							//	if (d0_KG_elektrona_rekonstr < 0.04 && z0_KG_elektrona_rekonstr < 0.1 && Energija_KG_elektrona_rekonstr > 100.0 && odnos_Ecal_TOTALcal_KG_elektrona_rekonstr >= 0.94){
								//if (d0_KG_elektrona_rekonstr < 0.1 && z0_KG_elektrona_rekonstr < 1 && Energija_KG_elektrona_rekonstr > 60.0 && odnos_Ecal_TOTALcal_KG_elektrona_rekonstr >= 0.95){
...
//								Brojac_za_dvojku = Brojac_za_dvicu;
							...
							//	if (d0_KG_pozitrona_rekonstr < 0.1 && z0_KG_pozitrona_rekonstr < 1 && Energija_KG_pozitrona_rekonstr > 60.0 && odnos_Ecal_TOTALcal_KG_pozitrona_rekonstr >= 0.95)	{
								temp_p = rekonstr_temp;
								leptoni.push_back(temp_p);
							//	niz_leptons.push_back(reclep_vector);
			...
			/*		if (leptoni.size()==2{ //elektron2 == true && pozitron2 == true)
				...
				for (Int_t j = 0; j < recParticles -> getNumberOfElements(); j++)
				{
					...
					bool provera = false;
					for (Int_t p = 0; p < reclep_vector.size(); p++)
					{
			...
			/////GMD//////
			 					Float_t coneEnergy, lepTrackEn, lepTrackEn_bss;

			if (reclep_vector.size()==2/*elektron2 == true && pozitron2 == true*/) {

				//////GMD ///////////

				for (int k = 0; (int) k < abs (reclep_vector.size()); k++)
						{
							coneEnergy = 0;

							TLorentzVector cone_lepton (TVector3 (reclep_vector[k]->getMomentum()), reclep_vector[k] ->getEnergy());
							TLorentzVector lepton_tlv (TVector3 (reclep_vector[k] ->getMomentum()), reclep_vector[k] ->getEnergy());
							pt_leptona = cone_lepton.Pt();
							p_leptona = cone_lepton.P();
							lepTrackEn = reclep_vector [k] ->getEnergy();
							thetaL = cone_lepton.Theta();
							pid_leptona = abs (reclep_vector[k] -> getType());
							//n_lep_evt++;

							for (Int_t p = 0; p < recParticles ->getNumberOfElements(); p++)
							{
								EVENT::ReconstructedParticle* pfop = (EVENT::ReconstructedParticle*) recParticles ->getElementAt(p);
								TLorentzVector pfop_LV (TVector3 (pfop->getMomentum()), pfop ->getEnergy());


								if (lepton_tlv == pfop_LV) continue;
								el_ph_angle = cone_lepton.Angle(pfop_LV.Vect()) * 180/M_PI;

								if ( pfop -> getType() == 22 && el_ph_angle <= 3)
								{
									cone_lepton += pfop_LV;
								}

							}// loop po pfo za BSS konus

							lepTrackEn_bss = cone_lepton.E();


							if (lepTrackEn_bss > 6)
							{ // Etrack cut
						//		if ( pid_leptona == 11) n_e++;
						//		if ( pid_leptona == 13) n_mu++;
							//	n_etrack++;
								std::vector<EVENT::Cluster*> clusters = (std::vector<EVENT::Cluster*>) reclep_vector[k] ->getClusters();
								Float_t ecal_en_emu = 0;
								Float_t hcal_en_emu = 0;
								for ( std::vector<EVENT::Cluster*>::const_iterator iCluster=clusters.begin(); iCluster!=clusters.end(); ++iCluster)
								{
									ecal_en_emu += (*iCluster) ->getSubdetectorEnergies()[0];
									hcal_en_emu += (*iCluster) ->getSubdetectorEnergies()[1];

								}
								ratio_en_emu = ecal_en_emu/(ecal_en_emu + hcal_en_emu);

								if ( (ratio_en_emu > 0.02 && ratio_en_emu < 0.35) || ratio_en_emu > 0.94)
								{ // Rcal cuts
					...
					//				if ( pid_leptona == 13) n_mu++;
							...
							//		n_etrack_rcal++;
									...
									const EVENT::TrackVec & trkvec = (const EVENT::TrackVec) reclep_vector[k]->getTracks();
									if (trkvec.size() > 0)
									{
										d0_recparticle = fabs (trkvec[0]->getD0());
										z0_recparticle = fabs (trkvec[0]->getZ0());
										r03d_recparticle = sqrt (d0_recparticle * d0_recparticle + z0_recparticle * z0_recparticle);
									}
									else{
										  d0_recparticle = 99;
										  z0_recparticle = 99;
									...
									if (r03d_recparticle < 0.03 && z0_recparticle < 0.03 && d0_recparticle < 0.02)
									{ // IP cuts
							//			if ( pid_leptona == 11) n_e++;
							//			if ( pid_leptona == 13) n_mu++;
						//				n_etrack_rcal_dz++;
									//	n_etrack_rcal_rdz++;
										...
										{
											EVENT::ReconstructedParticle* pfoj = (EVENT::ReconstructedParticle*) recParticles ->getElementAt(j);
											TLorentzVector pfoj_LV (TVector3 (pfoj->getMomentum()), pfoj ->getEnergy());
											bool ph = false;

											if (lepton_tlv == pfoj_LV) continue;
											if ( pfoj -> getType() == 22 )
											{
												ph = true;
												lep_pfoj_angle = cone_lepton.Angle(pfoj_LV.Vect())*180/M_PI;
											}
											else
											{ lep_pfoj_angle = cone_lepton.Angle(pfoj_LV.Vect())*180/M_PI; }

											if ( ph == true && cos (lep_pfoj_angle) > 0.995 && cos (lep_pfoj_angle) < 0.9986 ) { coneEnergy += pfoj_LV.E(); }
											else
											{
												if ( cos (lep_pfoj_angle) > 0.995 ) {coneEnergy += pfoj_LV.E();	}

											}
										}// loop po pfo za konus


								/*	if (powf(coneEnergy,2) < (powf(lepTrackEn_bss, 2) * (0) + lepTrackEn_bss *(20) - 20)) //{n_kriva++;}
									{
										n_kriva++;
										if ( pid_leptona == 11) n_e++;
										if ( pid_leptona == 13) n_mu++;

										vec_2isol.push_back(cone_lepton);

										histoTrackConeEnergy.Fill(lepTrackEn_bss, coneEnergy);
										ilfTree.Fill();
									}*/
